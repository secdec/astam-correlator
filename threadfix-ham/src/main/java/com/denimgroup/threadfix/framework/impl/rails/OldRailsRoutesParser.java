 ...
 */
public class OldRailsRoutesParser implements EventBasedTokenizer {
    ...
    public static Map parse(@Nonnull File file) {
        ...
        OldRailsRoutesParser parser = new OldRailsRoutesParser();
        EventBasedTokenizerRunner.runRails(file, true, false, parser);
...
//                            + "}/";
    ...
    public void processToken(int type, int lineNumber, String stringValue) {
        ...
        if (stringValue != null) {
            ...
            if (s.equals("ROOT") || s.equals("GET")) {
                ...
                currentRailsResource = new RailsResource(ResourceType.MATCH);
            }
        }
    ...
    private void processUrl(String method, int type, String stringValue) {
        ...
        String urlPrefix = buildCurrentUrlPrefix();
    ...
    String workingBinding = null;
    boolean isInQuote = false;
    private void processMatch(int type, String stringValue) {
        ...
        switch (currentRailsResource.getResourceState()) {
            case INIT:
                ...
                if (type == '\'' || type == '"') {
                    ...
                    if (stringValue != null) {
                        isInQuote = !isInQuote;
                        if (!isInQuote) {
                            workingBinding += stringValue;
                        }
                    }
                }

                if (isInQuote) {
                    workingBinding += CodeParseUtil.buildTokenString(type, stringValue);
                } else {
                    boolean isTo = (stringValue != null && (stringValue.equalsIgnoreCase("to:")
                            || stringValue.equalsIgnoreCase(":to")));
                    if (isTo || (type == '>' && lastType == '=')) {
                        currentRailsResource.setResourceState(ResourceState.TO);
                        currentRailsResource.setName(workingBinding);
                        workingBinding = null;
                    }
                }
                break;
            ...
            case TO:
                ...
                if (isInQuote) {
                    ...
                    boolean isVia = (stringValue != null && (stringValue.equalsIgnoreCase("via:")
                                                            || stringValue.equalsIgnoreCase(":via")));
                    if (isVia || (type == '>' && lastType == '=')) {
                        currentRailsResource.setResourceState(ResourceState.VIA);
                        currentRailsResource.setPath(workingBinding);
                        workingBinding = null;
                    }
                }
                break;
            ...
            case VIA:
                ...
                if (stringValue != null) {
                    ...
                    if (stringValue.equalsIgnoreCase("get") ||
                            stringValue.equalsIgnoreCase("put") ||
                            stringValue.equalsIgnoreCase("delete") ||
                            stringValue.equalsIgnoreCase("post")) {
                        String urlPrefix = buildCurrentUrlPrefix();

                        addMapping(stringValue.toUpperCase(), PathUtil.combine(urlPrefix, currentRailsResource.getName()));
                    }
                }
